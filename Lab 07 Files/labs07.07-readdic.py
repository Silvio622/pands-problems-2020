...
def readDict(): # create a function readDict 
    with open(filename, "rt") as f: # open the file "students.json" via filename variable
        return json.load(f) # load the values out of the student.json file
...
def displayMenu(): # create/ define a function with the name "displayMenu"
    ...
    print("\t (q) Quit")
    choice = input("Type one letter (a/v/s/l/q):").strip() # create a choice local variable as User input as string with strip function to clear leading and trailing spaces
...
def doLoad():
    ...
    global students # we are changing the local varibale student to global variable students
    students = readDict() # when the function readDict is called it loads the student.json file writes the values in global variable students
...
while (choice != 'q'): # the while loop loops as long the user doesn't put in "q"
    ...
    if choice == 'a': 
        ...
        doLoad() # if choice is equal to "l" then call the doLoad function
    elif choice == 'q':
